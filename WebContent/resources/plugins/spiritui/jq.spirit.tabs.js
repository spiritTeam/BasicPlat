...
//  var selectedTabs; //当前被选中的tab
  ...
  function doTabs(target, options) {
    ...
    for (;i<len; i++) {
      ...
      //查找是否已经有了tab
      if (to.length==0) {//未找到，直接画
        to=$("<div id='"+aTab.id+"'>"+aTab.title+"</div>");
        to.appendTo(target);
      }
      //得到文字宽度,高度
      var textPixelsLen=0, textPixelsHei=0;
      var fontTester=$("<div>"+aTab.title+"</div>");
      fontTester.addClass("tab").addClass(t_normal_cls);
      if (aTab.normalCss&&aTab.normalCss!="") fontTester.css(aTab.normalCss);
      fontTester.appendTo(target);
      textPixelsLen=fontTester.width();
      textPixelsHei=fontTester.height();
      fontTester.remove();
      //画标签
      ...
      to.css({"left": _tabLeft, "width": textPixelsLen+30, "padding-top": 0});
      to.spiritUtils("setHeightByViewHeight", $(target).height());
      if (($(target).height()-textPixelsHei)>0) to.css("padding-top", 1+(($(target).height()-textPixelsHei)/2));
